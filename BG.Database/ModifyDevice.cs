...
namespace BG.Database
{
    public class ModifyDevice
    {
        ...
        public DeviceSummaryVO SelectConfig(string id)
        {
            string sqlSelect_config = "select sno,model,sample_ratetype from tb_device_modify where id=?id";
            try
            {
                ...
                using (var connection = new MySqlConnection("server=localhost;user id=root;password=root;database=railway"))
                {
                    ...
                    using (MySqlCommand command = connection.CreateCommand())
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                DeviceSummaryVO modify = new DeviceSummaryVO();
